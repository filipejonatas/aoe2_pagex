...
describe('buildLeaderboard', () => {
  it('orders rating first and calculates snapshot deltas', () => {
    ...
    const rows = buildLeaderboard([
      { playerId: 'a', profileId: '1', nickname: 'A', country: null, rating: 1500, globalRank: 10, peakRating: 1510, isLeagueOwner: true, joinedAt: new Date(), snapshots: [{ rating: 1450, recordedAt: new Date(now - 8 * 86_400_000) }] },
    ...
    expect(rows[1].isLeagueOwner).toBe(true);
  });
});
