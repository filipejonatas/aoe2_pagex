...
export function RatingChart({ points }: { points: { label: string; rating: number }[] }) {
  const [range, setRange] = useState('30D');
  const path = useMemo(() => {
    const min = Math.min(...points.map((p) => p.rating)) - 20;
    const max = Math.max(...points.map((p) => p.rating)) + 20;
  ...
  return (
    <div className="chart-card card">
      ...
      <div className="chart-area" role="img" aria-label="Rating increased from 1628 to 1712 over 30 days">
